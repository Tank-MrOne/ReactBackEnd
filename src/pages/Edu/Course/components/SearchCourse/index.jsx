...
import {Card,Form,Input,Select,Button,Cascader} from 'antd'
import {reqAllNo1Subject,reqAllNo2SubjectById} from '@/api/edu/subject'
		...
		return (
			<Card>
				...
				>
					...
					>
						<Input 
							placeholder={intl.formatMessage({id:'title_show'})} 
							className="search_title"
						/>
					</Item>
					<Item label={<FormattedMessage id="teacher"/>} name="teacherId">
						<Select className="teacher_select">
							<Option key="-1" value="">{<FormattedMessage id="teacher_show"/>}</Option>
							{
								this.state.teachers.map((t)=><Option key={t._id} value={t._id}>{t.name}</Option>)
							}
						</Select>
					</Item>
					<Item label={<FormattedMessage id="subject"/>} name="subject">
						<Cascader
							placeholder={intl.formatMessage({id:'subject_show'})}
							options={this.state.no1SubjectList} //级联选择器的一级下拉菜单
